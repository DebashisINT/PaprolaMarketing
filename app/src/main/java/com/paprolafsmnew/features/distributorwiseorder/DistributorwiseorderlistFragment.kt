package com.paprolafsmnew.features.distributorwiseorder
...
//                fromDate.buttonTintList= ColorStateList.valueOf(resources.getColor(R.color.white))
    ...
    private fun saveDataAsPdf(objData: PdfDataRoot){
        ...
        val path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() + "/paprolafsmnewApp/DISTRIBUTORWISEORDERDETALIS/"
...
//                val widths = floatArrayOf(0.06f, 0.58f, 0.07f, 0.07f, 0.07f, 0.15f)
                ...
                var decPart = ""
                var finalConcatWord = ""
                tAmt = String.format("%.2f",tAmt.toDouble())
                if(tAmt.contains(".")){
                    intP = String.format("%.2f",tAmt.toDouble()).split(".").get(0).toString()
                    decPart = String.format("%.2f",tAmt.toDouble()).split(".").get(1).toString()
                    finalConcatWord = NumberToWords.numberToWord(intP.toDouble().toInt()!!)!!.toUpperCase()+" Rupees "+
                            NumberToWords.numberToWord(decPart.toDouble().toInt()!!)!!.toUpperCase()+" Paisa"
